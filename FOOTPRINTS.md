...
**Never derive a pin map from an unordered enumeration of signal names.** A datasheet sentence
naming the signals — "a 3-pin configuration (Counter, Reference and Working electrodes)", "pins are
VIN, GND, EN" — is a list of names, not an order. It is also exactly what `pdftotext` returns, so it
is the first thing a text-first workflow finds and the easiest thing to promote silently to a pin
map. Treat such a list as evidence that the pinout exists somewhere else in the document.

Any *explicit terminal relation* is admissible: a numbered pin-function table, a labelled pin
drawing, a polarity or marking statement tied to a drawn feature, a package standard, or a
validated measurement. **Record which relation you used.** The earlier form of this rule said
"never from prose", which is over-broad: Diodes DS30086 Rev. 31-2 p.1 (1N4148W, SOD123) carries no
numbered terminal table and establishes identity through "Polarity: Cathode Band" plus the marking
drawing — a literal never-from-prose rule makes that pin map impossible to establish
([`reviews/2026-09-01-codex-pin-identity-rules.md`](reviews/2026-09-01-codex-pin-identity-rules.md)
F1).
...
in top view. Take the view from an explicit TOP / BOTTOM / TERMINAL-SIDE label, or reconcile two
orthographic views against a named datum. If the view remains inferred, **record the inference and
mark the map `UNVERIFIED`** — never settle it by terminal visibility. Express the mirror as a
coordinate transform about a named axis and record its result, because an unstated view is an
unperformed mirror. Through-hole and bottom-terminated parts die here.

**Review correction.** This paragraph previously read "a package drawing that shows the pins as
visible features on a solid body is a bottom view, and its left-to-right order reverses when
transcribed." That heuristic is false and was disproved against a primary source: the same Diodes
DS30086 Rev. 31-2 p.1 labels a solid SOD123 with both metal terminals visible as **"Top View"**,
and its marking drawing puts the cathode band at the left, so classifying it as a bottom view and
mirroring moves the cathode to the right. Side elevations are a second counterexample
([`reviews/2026-09-01-codex-pin-identity-rules.md`](reviews/2026-09-01-codex-pin-identity-rules.md)
F2).
...
Require it **wherever the terminals are not electrically interchangeable**, independent of pad
count. A two-pad diode, LED, photodiode, unidirectional TVS, battery, or polarized capacitor fails
exactly as hard as a three-pin part when its terminals are swapped; the earlier "three pads or
more" threshold excluded that whole class
([`reviews/2026-09-01-codex-pin-identity-rules.md`](reviews/2026-09-01-codex-pin-identity-rules.md)
F3). Two interchangeable pads — an ordinary resistor, a non-polarized capacitor — need no map.

**Review correction — this check is not yet a guard, and presence of the fields must never pass.**
As written above the four fields are free text: `pinmap:` holds a document locator rather than the
map, `table:` holds a citation, and a wrong footprint passes verbatim with a confident but false
`leaders:` string. That is self-certification, and it fails [`GUARDS.md`](GUARDS.md)'s requirement
that a check observe the shipped artefact and be calibrated with a known-bad mutation. Until the
schema below exists, treat these fields as **provenance to read, not a check that passed**. What is
needed: a parseable post-transform mapping (`pad1=Counter; pad2=Working; pad3=Reference`) plus
source revision/hash, view, datum and transform, compared against the saved footprint's actual pad
numbers and the symbol's pin functions, and calibrated against every pin permutation plus the legal
top/bottom cases ([`reviews/2026-09-01-codex-pin-identity-rules.md`](reviews/2026-09-01-codex-pin-identity-rules.md)
F4). Not implemented.
...
a second independent document. Note also that a wrong map is not always rescuable by rotation: for an odd, uniform, collinear
inline pattern mounted in plane, 180° rotation maps position `i` to `N+1−i` and leaves the centre
position fixed, so a wrong centre assignment survives every allowed orientation. (For a five-pin
inline that is 1↔5 *and* 2↔4, not merely the outermost pair — the earlier "exchanges only its outer
pins" was wrong for N>3. Even-count, two-row and non-collinear patterns need their own symmetry
analysis; [`reviews/2026-09-01-codex-pin-identity-rules.md`](reviews/2026-09-01-codex-pin-identity-rules.md)
F10.)
...
**Review correction — that propagation is currently cosmetic.** No naming grammar,
saved-artefact checker, BOM/export rule or release assertion is defined anywhere in this skill, so
appending "unverified" to a library name removes a warning without proving that existing schematic
or board instances, or the fabrication outputs, carry it — a mute button by
[`GUARDS.md`](GUARDS.md)'s last test. What is needed: one machine-readable `PinmapStatus`
representation, verified in the saved footprint, the board instance, the schematic assignment and
the release manifest, and calibrated by a deliberately unverified fixture that release must refuse
([`reviews/2026-09-01-codex-pin-identity-rules.md`](reviews/2026-09-01-codex-pin-identity-rules.md)
F11). Not implemented — until it is, carry the deferral in the project's own release checklist.
