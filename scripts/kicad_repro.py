...
def _keep_name(output, suffix):
    """A backup name that is INJECTIVE over absolute paths.

    `<name>.tracked` collides whenever two outputs share a basename in
    different directories: with `keep_dir` set, two `board.kicad_pcb` files
    produced one backup, the second overwrote the first, and restoration put
    board B's bytes into board A while destroying A's only copy (codex review
    of 0f709ed). The path digest makes the name unique; the basename stays in
    front so a human can still read the directory.
    """
    tag = hashlib.sha256(str(output.resolve()).encode("utf-8")).hexdigest()[:12]
    return "%s.%s.%s" % (output.name, tag, suffix)
...
def check_frozen(cmd, outputs, cwd=None, env=None, algo="sha256",
    ...
    3. The generator runs, and generation is WITNESSED. **A generator that
       fails to run leaves the file untouched, which is byte-identical to a
       perfect pass** -- this module's founding lesson, and it applies with
       more force here than to the two-run check because there is only one run
       to prove. Default `witness="recreate"`: the verified copy is already
       taken, so the output is removed and the generator must put it back.
       `witness="mtime"` is the documented escape for a generator that edits
       in place instead of emitting; it only requires the mtime to move, which
       `os.utime` alone satisfies, so it does NOT witness generation and says
       so in its result.
    ...
    for o in outs:
        ...
        keep = (scratch or o.parent) / _keep_name(o, "tracked")
    ...
    def _restore():
        ...
        for o, (d0, keep, _) in saved.items():
            try:
                # Verify the copy AT THE MOMENT OF USE, not only when it was
                # taken: between those two points a generator, another run, or
                # a colliding backup name may have changed it. Restoring
                # unverified bytes over a tracked artefact is worse than
                # refusing to restore.
                if digest(keep, algo) != d0:
                    problems.append(
                        "%s (verified copy %s no longer hashes %s -- NOT "
                        "restored)" % (o, keep, d0))
                    continue
    ...
    # mtime movement is NOT proof of generation: a command that only called
    # `os.utime(output, None)` exited 0 and passed this check (codex review of
    # 0f709ed), which contradicts SKILL.md's own rule against mtime-only
    # proof. The default witness is therefore destructive-and-recreate: the
    # verified copy is already taken, so the output is removed and the
    # generator must put it back. A generator that does not write cannot.
    if witness not in ("recreate", "mtime"):
        raise ReproError(
            "witness=%r is not a mode (choose 'recreate' or 'mtime')"
            % (witness,))
    if witness == "recreate":
        for o in saved:
            try:
                ...
                _fail(ReproError,
                      "cannot remove %s to require the generator to recreate "
                      "it (%s); pass witness='mtime' knowingly if this "
                      "generator edits in place rather than emitting" % (o, e))
    ...
    for o, (_, _, mt0) in saved.items():
        ...
        if witness == "mtime" and o.stat().st_mtime_ns == mt0:
            _fail(ReproError,
    ...
    for o, (d0, keep, _) in saved.items():
        now = digest(o, algo)
        if now != d0:
            regen = (scratch or o.parent) / _keep_name(o, "regenerated")
            try:
                shutil.copyfile(o, regen)
                regen_bytes = Path(regen).read_bytes()
            except OSError as e:
                # Keeping the evidence is a courtesy; RESTORING is the
                # promise. An unwritable or already-a-directory target used to
                # raise here, skip _fail() entirely, and leave the tracked
                # artefact regenerated with an uncaught OSError (codex review
                # of 0f709ed).
                regen = "%s (could not be saved: %s)" % (regen, e)
                regen_bytes = Path(o).read_bytes()
            off, line, was, isnow = _first_difference(
                Path(keep).read_bytes(), regen_bytes)
            changed.append(
                "%s\n    tracked      %s  (%s)\n    regenerated  %s  (%s)\n"
    ...
    if mode == "digest":
        ...
        if mode == "check":
            ...
            try:
                ...
                # The exit contract is 0/2/3; an escaping OSError was exit 1
                # with a traceback and no statement about the artefact.
                sys.stderr.write(
                    "frozen check could not run: %s\n" % e)
                raise SystemExit(2)
    else:
