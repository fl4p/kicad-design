...
| `kicad_drc_connectivity.py` | fail-closed classifier for a fresh full-severity KiCad JSON DRC report that does not ignore `unconnected_items`: exact `--pour-net` names make every record on those pure-pour nets topology; `--mixed-pour-net` keeps every record ambiguous when authored routing and refill-owned work share a net; `--no-pour-nets` explicitly declares none. Non-pour records without zones are signal opens; zones on undeclared nets plus missing/mismatched net tokens remain ambiguous. Reports signal, pour, ambiguous and aggregate totals; a total AT OR ABOVE the per-type report cap is unevaluable — `==` broke monotonicity (198 passed, 199 was unevaluable, 200 passed again), and the 199 itself was measured for `silk_overlap`/`silk_over_copper` on KiCad 10.0.5, never for `unconnected_items`; `--report-cap N` is the escape once a release has actually been probed — and it must carry that evidence: any value other than the default requires `--report-cap-probed-on <version>` matching the report's `kicad_version` **and** `--report-cap-probe FILE`, a report from that release actually containing more findings of a single `type` than the cap being claimed. `none` is no longer accepted at all: a probe of M findings proves the cap is at least M and is equally consistent with the cap being exactly M, so a bounded observation can justify a higher finite cap and never "no cap". The probe counts per `type`, ignores non-object rows, and may not be the report being graded — a report cannot be its own evidence that it was not truncated. A malformed value is a configuration error, not a disabled check; Fail closed on the gate as well as the measurement: `--require-zero-signal-opens` grades the signal side alone -- and tightens classification while it does, because the pour declaration is caller-supplied and is not evidence: This gate ranks progress and closes the *signal component*; it is **not** the definition of done — [`../ROUTING.md`](../ROUTING.md) keeps that at `--require-zero-total`, because an island is unfinished copper until stitched or removed, and a run that reaches zero signal opens reports "signal routing complete, board incomplete". Under that gate only an **all-zone** record — a zone island, which is what a refill actually produces — counts as pour topology; a record naming any pad, track or via stays **ambiguous** (exit 3), because copper stranded from its pour is authored routing and the DRC text cannot tell it from a refill artefact. Requiring merely that a zone be PRESENT was not enough: a `Zone [/SIG] ↔ Track [/SIG]` record on a net declared `--pour-net /SIG` still exited 0. Because real pour nets genuinely do carry zone-track, zone-via and track-via records, the escape is per-record rather than per-net: `--reviewed-record <id>` acknowledges ONE record by the `record_id` the JSON reports, so each excuse is a specific auditable claim, it cannot excuse an undeclared net, and an id matching no record in the report is refused rather than carried forward past the record it was about. It can only narrow a record that **actually names a zone**: as first shipped it lacked that clause and reopened the exact laundering the all-zone rule had just closed — a plain `Pad [/SIG] ↔ Track [/SIG]` open with no zone anywhere exited 0 once the caller supplied both a `--pour-net` and an acknowledgement. Two caller-supplied labels are two statements, not two pieces of evidence; the zone is the only physical evidence in the record. Relabelling a real open's net as pure pour turned exit 4 into exit 0 before this — the gate a board with legitimate pour records can actually use, since `--require-zero-total` can never pass on one — `--require-zero-total` gates the original aggregate completion criterion, and a run with neither gate and no explicit `--report-only` is a configuration error (exit 2), not a pass. **A gating run must either produce its own report (`--run-drc`) or say explicitly that it is trusting one it did not make (`--trust-external-report`).** Measured 2026-09-07: a board with 11 real opens exited 0 when handed a zero-item report from a *different* board that merely shared its filename and carried a later timestamp. Basename equality plus a timestamp ordering is not a binding, and no amount of metadata checking makes it one — under `--run-drc` the tool digests the board, invokes `kicad-cli` on that exact file and grades the output it just produced, which is the only mode where the verdict is tied to the board's bytes. The weaker path stays available but is opt-in and recorded in `drc_provenance` alongside every other escape used, so a pass taken on a weak footing is auditable rather than indistinguishable. A gating run must also name the board with `--board`: `source_sha256` binds the report's own bytes, never the copper they describe — without it a zero-open report for a *different* design exited 0. The board is digested into the result (`board.board_sha256`), the report's own `source` must name it, and every declared pour net must be a net the board actually has, so a misspelled `--pour-net` can no longer declare nothing silently. `--report-only` may still inspect a report on its own. `source` and `date` are now required report keys; both are present in every real KiCad 10.0.5 export, and `date` is parsed and compared against the board's mtime — a report taken BEFORE the board was last modified cannot describe its current bytes and is refused, with `--allow-report-older-than-board` as the explicit override — which suppresses only that one comparison: the timestamp is still parsed and a report dated more than an hour ahead is still refused, since the escape used to skip the whole check and a report dated 2099 exited 0 (a two-second tolerance covers KiCad's second-resolution timestamp against a nanosecond mtime). Every escape in force is printed to **stdout** beside the verdict as `PROVENANCE:` lines, not only into the optional `--json`, so a CI log shows when a pass rested on a trusted external report, a stale-report allowance, a cap override or a reviewed record — an acknowledgement given without a reason says so explicitly. The assumed timezone is recorded too, because a naive KiCad timestamp is read as local time and the same stale report is refused under one zone and accepted under another. The board's net names are read by an s-expression scan, not a text grep, and it matches BOTH spellings KiCad uses: a KiCad 8 board carries a numbered table (`(net 3 "/SIG")`) while a KiCad 10 board has no top-level net section at all and names a net only as a bare reference (`(net "/SIG")`) inside the pad, segment, via or zone that carries it. Measured over 400 real boards: matching only the numbered form returned an EMPTY inventory for every KiCad 10 board, which would have refused every legitimate `--pour-net` on the release this skill targets. What the scan yields is therefore "every net this board names", which is the right question for catching a misspelled declaration; a regex counted the same expression inside a pad, inside a comment, and inside a file that was not a board at all, so a garbage file could authorise a `--pour-net`. A board whose parens do not balance, or whose root is not `kicad_pcb`, is refused, and an oversized file is refused rather than read. The JSON records `signal_open_gate` and `aggregate_gate` separately from measurement evaluability. Accepts KiCad's `mm`, `in`, and `mils` JSON coordinate units; calibration: `test_kicad_drc_connectivity.py`, including an artefact tier — `fixtures/open-net.kicad_pcb` is a real board with two real opens and `fixtures/open-net.drc.json` is its real `kicad-cli pcb drc` export, re-run against the installed KiCad when one is present so a release that changes the report format is noticed rather than silently tolerated |
