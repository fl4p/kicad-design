...
1. REOPENED (was FIXED) — Body grep found no surviving project refdes, private net names, session names, `+5VH`, `GND_OUT`, or `krt_fab_floor.txt`. That half stands. The other half of the original closure — “the remaining board-specific measured numbers are covered by the owner’s stated allowance” — does not: **that allowance is recorded nowhere in this repository.** A grep over `reviews/` and every doc finds only the sentence claiming it. Contrast the CELL_WE waiver at `2026-09-05-cross-session-routing-evidence.md:136`, which is quoted verbatim (`i never put these "CELL_WE and guard" rules, i dont care. if it works..`) and is about a different subject entirely — the CELL_WE and guard-replica locks, not project-agnosticism. A blocking finding closed on an unquotable waiver is closed on nothing, and writing the missing quote now would be inventing the owner’s words. So the finding is reopened, not re-closed: the board-specific numbers stay in the body on the stated engineering ground that a worked example needs real numbers, hedged in place as “Measured once, by one agent … not replicated” — which is an argument, and can be argued with. If the owner did grant a general allowance, quote it here and this reverts to FIXED. Reopened per finding 10 of `2026-09-07-claude-review-a5c83c2-7a99de9.md`.
