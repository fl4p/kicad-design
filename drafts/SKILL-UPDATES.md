...
**Three in a row, which is the part that generalises.** The same shape recurred a third time on the same router. A second cache — one that records *failed* route attempts — is sound only while copper is added and never removed, with one exception: an endpoint that gains a via of its own net becomes easier to route, so a recorded failure can stop being true. That dependency went into the cache key. Deleting it again — the deliberately unsound key — produced an **identical board**, on an input where the condition occurs **62 times in 549 calls**. Exercised, reasoned-correct, and unfired.

So: invalidation on the segment cache, `has_via` in the failure key, and the first calibration attempt on each — three mechanisms, three calibrations that would not fire. That rate is itself a finding. When a system's *state space* is narrow — here, a single deterministic board generated from fixed inputs — most guards protecting against state transitions have nothing to fire on, and a calibration suite run only against that state will report a clean sweep while testing almost none of them. The response is not to delete the guards but to **stop counting unfired calibrations as evidence, and to source the argument elsewhere** — from review, from a proof, or from a second input with different state. The `has_via` flags and the invalidation both survive on structural argument, not on any test that passed.
