...
* **keep the code, drop the claim.** The invalidation stays, because correctness resting on "the caller happens not to reuse a key after a commit" is a property of today's call order rather than of the cache, and the first reordering of those calls yields a silent stale PASS. But it is **unfired**, and the write-up must say so instead of listing it among the calibrated guards;
* **do not let the non-firing calibration read as a pass.** "I tried to break it and couldn't" is the sentence that turns an untested mechanism into a trusted one. What was actually established is narrower: *on this input, the mechanism is never exercised.* Absence of evidence, again — this time about the guard rather than about the board.
